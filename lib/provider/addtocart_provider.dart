...
class AddtoCartProvider with ChangeNotifier {
      ...
      if (cartDoc.exists) {
        ...
        for (final cartItem in cartItemsList) {
          ...
          if (itemDocSnapshot.exists) {
            ...
            final item = ItemModel(id: productId, name: data['name'], price: data['price'], 
                  img: data['img'],brand: data['brand'],category: data['category'],quantity: quantity);
            cartItems.add(item);
          }
        }
  ...
  Future<void> placeOrder(BuildContext context, AddtoCartProvider cartProvider, String userId) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Your cart is empty. Add items to your cart before placing an order.')));
      return;
    }
      ...
      for (final item in cartItems) {
        ...
        if (item.quantity > availableQuantity || item.quantity<1 || availableQuantity<1 || !itemDoc.exists || brandItemsMap.isEmpty || cartProvider._cartItems.isEmpty) {
          canPlaceOrder = false;
          break;
        }
      }
      if (canPlaceOrder) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Order cannot be placed. Item quantity exceeds availability.')));
      }
    } catch (e) {
